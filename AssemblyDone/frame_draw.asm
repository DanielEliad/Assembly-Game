...
createarrowing db 'I am creating a bullet',0
...
NextWaveP ENDP
 ...
 SendInformation PROC, paramter:DWORD
	...
	real_endofSendInformation:
	invoke Sleep,3
	jmp again
	notagain:
	ret
SendInformation ENDP
...
xor edx,edx
imul ecx
invoke SetTimer, hWnd, RoundEnded, eax, NULL ;Set the time til the store
jmp finish
notresume:
...
invoke DrawImage_WithMask,hdc,eax,edx,ZombieX,ZombieY,30,40,dword ptr [ebx+28],0,Zombie_Width,Zombie_Height
invoke GetTickCount
...
drawArrow PROC,hdc:HDC,brush:HBRUSH,lp_bullets_array:DWORD,hWnd:HWND
;--------------------------------------------------------------------------------
local rc:RECT
local testrc:RECT
FNINIT
...
invoke SetWorldTransform,hdc,offset normal_xForm
popa
pusha
...
Draw_Sprint_Bar ENDP
 ...
 ;--------------------------------------------------------------------------------
		local pt:POINT 
           ...
           FLD dword ptr [esi+8];Float Load Delta Y
		   FLD dword ptr [esi];Float Load Delta X
		    
           FPATAN;Compute Tangent
           FSINCOS
		   FMUL VEL
            ...
            .if ax==NULL 
				 ...
				 .if eax==NULL 				    
					...
					.if expecting_PORT == TRUE
					 ...
					 invoke CreateThread, NULL, NULL, offset SendInformation,offset clientsin, NULL, NULL
 ...
 movss normal_xForm.ey,xmm0
	   ...
	   ret
        ...
        painting:
			...
			cmp two_Players,FALSE
			je dontdrawplayer2
			invoke BITMAP_ID_TO_HBITMAP,117
			invoke DrawImage_WithMask,hdcBuffer,eax,edx,Player2.x,Player2.y,75,105,Player2.CURRENTSTEP,Player2.CURRENTFACING,RECT_WIDTH,RECT_HEIGHT
			dontdrawplayer2:
        ...
        invoke drawArrow,hdcBuffer,brushcolouring,offset bullets,hWnd;MAYBE REMOVE
		invoke SetDCBrushColor,hdcBuffer,0000000F87FFh
		invoke drawArrow,hdcBuffer,brushcolouring,offset enemybullets,hWnd;MAYBE REMVOE
...
timing:
		   ...
		   ;cmp host,TRUE
		   ;je noneedtodebug
		   ;invoke MessageBox,0,offset createarrowing,offset createarrowing,MB_OK
		   ;noneedtodebug:
		   invoke createarrow
		   jmp EndingTime
		   nexting:
